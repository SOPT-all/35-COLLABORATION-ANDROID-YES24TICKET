    ...
    fun toTimeSlotsData() = TimeSlotsDataEntity(
        slots = data.map {
            TimeSlots(
                ...
                remainingSeats = it.remainingSeats.map {
                    RemainingSeat(
                        type = it.type,
...
data class ResponseTimeSlotsDto(
    ...
    @SerialName("remaining_seats") val remainingSeats: List<ResponseRemainingSeatDto>
)

@Serializable
data class ResponseRemainingSeatDto(
    @SerialName("type") val type: String,
    @SerialName("remaining") val remainingSeats: String
)
